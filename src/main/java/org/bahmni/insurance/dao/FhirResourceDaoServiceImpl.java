...
public class FhirResourceDaoServiceImpl implements IFhirResourceDaoService {
	...
	public int insertFhirResource(String fhirResource, String type) throws DataAccessException {
		return jdbcTemplate.update(
				"INSERT INTO fhir_resource( ts, version, resource_type, status, resource) "
						+ "VALUES (?,?,?,?,CAST(? as jsonb))",
				new Date(), ImisConstants.FHIR_VERSION, type, "Requested", fhirResource);
	...
	public String getClaimRequestByClaimId(String claimId) {
		...
		if (result.size() > 0) {
			...
			throw new RuntimeException("No Claim Request Resource found ");
		}
